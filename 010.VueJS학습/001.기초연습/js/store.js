...
/*************************************************************** 
    ...
    3. 컴포넌트 구조 상태와 상관없이 사용하는곳에서 변경한
        내용이 동기화되어 자연스러운 업데이트가 이루어진다!
    ...
    - 흐름 : 먼저 비동기 메서드 처리 후
            일반 메서드 처리 결과를 state변수에 반영함
    (참고: https://docs.google.com/document/d/1xT3IMlWO_kY7pJqNJSDpI4IPe6BtpfoYq9r5M5_YiRE/edit)
    ...
    actions/mutations:{
        ...
        -> 첫변수는
            mutation은 뷰엑스 스토어의 state변수가 자동으로 들어옴!
    ...
    [ 뷰엑스 스토어 버전이슈! ]
    - 일반적으로 뷰JS 버전이 2.xx이면 vuex는 3.xx 버전을 씀
    (예컨대 최신 뷰는 3.x이므로 vuex는 4.x버전을 씀)
    -> 메이저버전 숫자가 뷰보다 하나크다!
    (주의 : webpack 템플릿을 사용한 SPA구축환경에서

...
function Member() {
    ...
    // [ 리책트 라우터 이동 시 이동 메서드 이용하기 : useNavigate ]
    ...
    const msgId = [
        "User ID must contain a minimum 5 letters",
    ...
    const changeUserId = (e) => {
        ...
        if (valid.test(e.target.value)) {
            ...
            // 아이디 형식에는 맞지만 아이디가 사용중인 아이디인지 검사
            let memData = localStorage.getItem("mem-data");
            console.log("로컬쓰",memData)
            // 로컬쓰 null아닌 경우
            if(memData){
                // 로컬쓰에 기존 아이디 중 있는지 확인하기
    ...
    const onSubmit = e=>{
        ...
        if(totalVaild()){
            ...
            // 로컬스 변수할당
            let memData = localStorage.getItem("mem-data")
            console.log("전",memData)
            
            // 로컬스 객체로 변환하기
            memData = JSON.parse(memData);
            console.log("후",memData)
                    ...
                    <ul>
                        ...
                        <li>
                            ...
                            {pwdError && (
                                <div className="msg">
                                    <small style={{ color: "red", fontSize: "10px" }}>
                                        Password must ve at least 8 characters long and myst contain at least one letter and one capital word.
                                    </small>
                                </div>
                            )}
                        </li>
                        ...
                        <li style={{overflow:"hidden"}}>
                            ...
                            {/* input submit버튼이 아니어도 form요소 내부의 button은 submit 기능이 있다 */}
                        </li>
                        <li>
                            {/* 7.로그인페이지링크 */}

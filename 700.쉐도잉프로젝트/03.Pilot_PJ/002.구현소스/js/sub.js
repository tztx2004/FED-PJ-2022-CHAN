...
(()=>{
    ...
    if(location.href.indexOf("?")!==-1)
        ...
        // 물음표(?)로 잘라서 뒤엣것, 이퀄(=)로 잘라서 뒤에 것
        // 파라미터 값만 추출함!
    // pm에 할당이 되었다면 undefined가 아니므로 true
    if(pm)
        store.commit('chgData',decodeURI(pm))

    // 메뉴를 선택해서 파라미터롤 들어오지 않으면 "남성"셋팅
    else
        store.commit('chgData',decodeURI("남성"))
...
new Vue({
    ...
    mounted: function () {
        ...
        // GNB 메뉴 클릭 시 해당위치로 스크롤 이동 애니메이션
...
function sinsangFn() {
    ...
    **********************************************/
    ...
    function moveList() {
        ...
        // 조건 : call_sts===1
        if(call_sts===1) callT = setTimeout(moveList, 40);
    }

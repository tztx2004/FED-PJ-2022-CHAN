...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        updateMore(dt,pm){ // pm - 업데이트할 전달숫자
            // pnum은 리스트 범위수 : +=로 여러번 모어진행
            dt.mnum += pm;


            // 업데이트 후 모어버튼 사라짐(한계수를 넘으면)
            if(dt.mnum>=25)
            dt.mbtn = false;
        },///////////////// updateMore //////////////
        //////// [ 장바구니 데이터 업데이트 메서드 ]
        setData(dt,pm){
            ...
            org.forEach(v=>{
                ...
                // dt.gdata[pm].idx는 입력하려는 새로운 아이템idx
            ...
            if(save){
                ...
                console.log("반영 후 로칼쓰",localStorage.getItem("cart"))
    
                // 5. 카트 애니메이션 버튼을 등장시켜 카트리스트 까지 연동한다!
        ...
        delRec(dt,pm){
            ...
            // 3. 삭제아이템 찾아서 지우기 : splice(순번,개수)
            org.forEach((v,i)=>{
                // 지울아이템과 같으면
                if(v.idx === pm){
                    // 지울것인지 물어봄(확인 true/취소시 false)
                    if(confirm("정말정말정말로 지우시겠습니까? 할인도 하는데?")){
                        org.splice(i,1);
                    }
        ...
        bindData(dt,pm){ // pm - 카트박스 right값 전달
            ...
            let rec = org.map((v,i)=> // v - 배열 각 값 / i - 배열순번
                    `<tr>
                        ...
                        <td>
                            <img
                            src="${'images/goods/'+v.cat+'/'+v.ginfo[0]+'.png'}" style="width:50px"
                            alt="item" />
                        </td>
                        <!-- 번호 : 리스트순서번호 -->
                        <td>${i+1}</td>
